...
def parse_args(*k, **kw):
    ...
    for option in get_args().option:
        ...
        if len(l) == 2:
            ...
            logger.error("Option malformated '%s', it needs to be formated 'key=value'" % option)
            sys.exit(1)
    ...
    for option in get_args().option:
        ...
        if len(l) == 2:
            ...
            if not parameter.exists(name):
                logger.error("Option does not exist: %s" % (name))
                malformated = True
    if malformated:
        logger.error("The execution is stoped due to malformated options")
        sys.exit(1)
